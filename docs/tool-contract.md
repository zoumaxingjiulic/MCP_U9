...
- 输入：必填 `organization_code`、`item_code`，均为非空字符串，保留前导零；拒绝未知字段、通配符和首尾空白。长度上限 100 是本服务限制，不宣称为 U9 上限。
- 用途：在服务配置的企业和唯一授权组织中，按准确料号获取料品档案。没有模糊搜索、跨组织、库存数量或金额统计。
- 身份：stdio 或 HTTP 服务进程固定绑定一个 ERP 应用/用户/企业/组织；调用者共享该身份，不提供多人身份隔离。HTTP 入口要求独立 Bearer 令牌。每次调用校验输入组织，并再次核对 ERP 返回组织和料号。
