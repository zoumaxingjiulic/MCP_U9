...
class ItemQuery(StrictModel):
    organization_code: str | None = Field(
        ...
        min_length=1,
        max_length=100,
        description="可省略，自动使用服务端已授权组织；仅供旧客户端核对，不能切换组织。",
    )
    ...
    def exact_code(cls, value: str) -> str:
        if value is None:
            raise ValueError("请省略组织编码以使用服务端配置，不要传入 null")
        if value != value.strip() or any(ord(c) < 32 for c in value) or any(c in value for c in "*%?"):
            raise ValueError("必须提供准确编码，不支持通配符或首尾空白")
        return value


class ResolvedItemQuery(ItemQuery):
    organization_code: str = Field(min_length=1, max_length=100, description="实际使用的服务端授权组织")
